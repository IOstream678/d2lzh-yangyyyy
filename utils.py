...
def load_data_jay_lyrics(full=False):
...
def show_images(imgs, num_rows, num_cols, scale=2):
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            axes[i][j].imshow(imgs[i * num_cols + j].asnumpy())     # 本身axes的imshow函数就是通道数在第三维
...
def train(train_iter, test_iter, net, loss, trainer, ctx, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_iter):
            ...
            sys.stdout.write('\rCalculate %d of %d batches in epoch %d, current loss %.4f, train acc %.3f' % (i + 1, len(train_iter), epoch + 1, train_l_sum / n, train_acc_sum / m))
            sys.stdout.flush()
...
def train_and_predict_rnn(rnn, get_params, init_rnn_state, num_hiddens,
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in data_iter:
            ...
            n += y.size
        sys.stdout.write('\rProcessed %d of %d, perplexity %f' % (epoch + 1, num_epochs, math.exp(l_sum / n)))
        sys.stdout.flush()
...
def train_and_predict_rnn_gluon(model, num_hiddens, vocab_size, ctx,
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in data_iter:
            ...
            l_sum += l.asscalar() * y.size
            n += y.size
        sys.stdout.write('\rTrained %d of %d, perplexity %f' % (epoch + 1, num_epochs, math.exp(l_sum / n)))
        sys.stdout.flush()
        if (epoch + 1) % pred_period == 0:
            print('\nepoch %d, perplexity %f, time %.2f sec' % (
                epoch + 1, math.exp(l_sum / n), time.time() - start))
